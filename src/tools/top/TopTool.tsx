...
import { SCPIS, TD_ATYPIQUE, type Scpi } from '../../lib/scpi'
import { usufruitOpportunities, type Opportunity } from '../../lib/usufruit'
import { StatutBadge } from '../cles/ClesTool'

const TOP_N = 3

/** Classement du marché : meilleur TRI 100 % usufruit par durée (donc par clé), toutes SCPI confondues. */
export function TopTool({ onUse }: { onUse: (scpi: Scpi, duree: number) => void }) {
  ...
  const all = useMemo(() => usufruitOpportunities({ delaiJouissanceMois: delai }), [delai])
  const rows = all.filter(
    (o) =>
      o.tri != null &&
      !(hideClosed && o.scpi.statut) &&
      !(hideUncertain && o.scpi.fiabilite === 'incertain') &&
      !(hideAtypical && o.td > TD_ATYPIQUE),
  )
  ...
  const selected = duree ?? byDuration.find(([d]) => d === 5)?.[0] ?? byDuration[0]?.[0] ?? null
  const ranking = byDuration.find(([d]) => d === selected)?.[1] ?? []
  const excluded = SCPIS.filter((s) => s.td == null).length
    ...
    <div className="mx-auto flex max-w-[1400px] flex-col gap-6 px-4 py-6 sm:px-8 sm:py-8">
      <header>
        ...
        <p className="mt-1 max-w-3xl text-sm text-slate-500">
          Pour chaque durée, donc chaque clé, TRI d’un investissement 100 % usufruit calculé avec la clé et le TD publiés de chaque
          SCPI. C’est un repérage des prochains investissements usufruit potentiels, à confirmer auprès de la société de gestion
          (clé en vigueur, TD prévisionnel, disponibilité des parts).
          {excluded > 0 && ` ${excluded} SCPI sans TD publié sont exclues.`}
        </p>
      </header>
      ...
      <section className="flex flex-wrap items-end gap-x-6 gap-y-3 rounded-2xl border border-slate-200/80 bg-white px-5 py-4 shadow-xs">
        <div className="w-44">
          <NumberField label="Délai de jouissance" value={delai} onChange={(v) => setDelai(Math.round(v))}
            suffix="mois" min={0} max={36} step={1} />
        </div>
        ...
        <Toggle label={`Exclure les TD atypiques (> ${TD_ATYPIQUE * 100} %)`} checked={hideAtypical} onChange={setHideAtypical} />
      </section>
      ...
      <section className="rounded-2xl border border-slate-200/80 bg-white shadow-xs">
        <div className="px-5 pb-3 pt-4">
          ...
          <p className="mt-0.5 text-xs text-slate-500">Cliquez une durée pour voir le classement complet.</p>
        </div>
                      ...
                      <td key={i} className="px-3 py-2">
                        {o ? (
                          <span className="flex items-baseline gap-2">
                            ...
                            <span className="truncate text-xs text-slate-600">{o.scpi.nom}</span>
      ...
      {selected != null && ranking.length > 0 && (
        <section className="rounded-2xl border border-slate-200/80 bg-white shadow-xs">
          <div className="flex flex-wrap items-end justify-between gap-3 px-5 pb-3 pt-4">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Classement complet — {selected} ans</h2>
              <p className="mt-0.5 text-xs text-slate-500">
                {ranking.length} SCPI · « Comparer » ajoute l’investissement au comparateur TRI usufruit.
              </p>
            </div>
            ...
            <table className="w-full text-sm tabular-nums">
              <thead className="bg-slate-50 text-xs text-slate-500">
                <tr>
                  ...
                  <th className="px-3 py-2.5 text-right font-medium">TRI usufruit</th>
                  <th className="px-3 py-2.5 text-right font-medium">Clé usu</th>
                  <th className="px-3 py-2.5 text-right font-medium">TD</th>
                  <th className="px-3 py-2.5 text-right font-medium">Prix de part</th>
                ...
                {ranking.map((o, i) => (
                  <tr key={o.scpi.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50">
                    ...
                    <td className="whitespace-nowrap px-3 py-2 text-right text-slate-700">
                      {fmtPct(o.td)}
                      {o.td > TD_ATYPIQUE && <span className="text-amber-600" title="TD atypique : à vérifier"> ⚠</span>}
                      {o.scpi.anneeTd && <span className="ml-1 text-[10px] text-slate-400">{o.scpi.anneeTd}</span>}
                    </td>
                    <td className="px-3 py-2 text-right text-slate-500">{o.scpi.prixPart ? fmtEur(o.scpi.prixPart) : '—'}</td>
                    ...
                    <td className="px-5 py-2 text-right">
                      <button onClick={() => onUse(o.scpi, o.duree)}
